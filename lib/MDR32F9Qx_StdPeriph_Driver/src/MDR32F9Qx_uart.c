...
/**
  ...
  *             1, 2 for MDR1986VE9x and MDR1986VE1;
  *             1, 2, 3 for MDR1901VC1;
  *             1, 2, 3, 4 for MDR1986VE3.
  * @retval None
  */
...
#endif
{
    ...
    else if((UARTx == MDR_UART1) || (UARTx == MDR_UART2))
#endif /* #if defined (USE_MDR1986VE3) */
    ...
    {
        cpuclock /= (1 << ((tmpreg & UART2_BRG_Mask) >> UART4_BRG_Offs));
    }
#endif /* #if defined (USE_MDR1986VE3) */
...
void UART_BRGInit(MDR_UART_TypeDef* UARTx, UART_Clock_BRG_TypeDef UART_BRG)
{
    ...
    else if(UARTx == MDR_UART3)
    {
        ...
        tmpreg |= (((uint32_t)UART_BRG) << RST_CLK_UART_SSP_CLOCK_UART3_BRG_Pos);
    }
    else if(UARTx == MDR_UART4)
    {
        tmpreg |= RST_CLK_UART_SSP_CLOCK_UART4_CLK_EN;
        tmpreg &= ~RST_CLK_UART_SSP_CLOCK_UART4_BRG_Msk;
        tmpreg |= (((uint32_t)UART_BRG) << RST_CLK_UART_SSP_CLOCK_UART4_BRG_Pos);
    }
#endif
